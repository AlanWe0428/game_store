...
try:
    ...
    # --- 重要：欄位自動補齊機制 (防止舊資料導致 IndexError) ---
    required_cols = {
        "特定球員": "",
        "下注總額": 0,
        "領回金額": 0,
    ...
    st.error(f"❌ 讀取失敗：請檢查試算表欄位或 Secrets 設定。")
    st.stop()
...
with st.expander("📝 錄入新注單", expanded=False):
    ...
    with st.form("multi_form", clear_on_submit=True):
        ...
        for i in range(int(num_m)):
            ...
            with c_vs: 
                st.markdown("<p style='text-align:center; padding-top:35px;'>VS</p>", unsafe_allow_html=True)
            with c_t2:
            ...
            p_sel = c_p1.selectbox(f"從歷史選球員", player_options, key=f"ps_{i}")
            p_new = c_p2.text_input(f"或手輸新球員", key=f"pn_{i}")
...
# --- 4. 老師切換按鈕區 (恢復此段邏輯) ---
st.write("👤 **切換老師視角：**")
teacher_cols = st.columns(len(all_teachers))
for i, teacher_name in enumerate(all_teachers):
    with teacher_cols[i]:
        # 如果是目前選中的老師，按鈕顏色變深 (primary)
        btn_type = "primary" if st.session_state.current_teacher == teacher_name else "secondary"
        if st.button(teacher_name, key=f"t_btn_{teacher_name}", use_container_width=True, type=btn_type):
            st.session_state.current_teacher = teacher_name
            st.rerun()
...
if not df_display.empty:
    ...
    c2.metric("累積損益", f"${profit:,.0f}", f"總投資 ${t_cost:,.0f}")
